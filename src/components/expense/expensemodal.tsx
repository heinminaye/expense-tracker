...
const ExpenseModal: React.FC<ExpenseModalProps> = ({
  ...
  const handleBreakdownItemChange = (
    ...
    if (field === "quantity") {
      ...
      value = stringValue === "" ? 0 : parseInt(stringValue, 10);
    }
  ...
  return (
    ...
    >
      <div
        className={`rounded-xl shadow-xl w-full max-w-4xl flex flex-col min-h-[80vh] max-h-[80vh] overflow-hidden transition-all duration-300 transform ${
        ...
        <div className="overflow-auto max-h-[280px] flex-1">
          <table className="min-w-full divide-y text-md">
            <thead className={`sticky top-0 ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}>
              <tr>
                <th className={`px-2 py-2 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? "text-gray-300" : "text-gray-500"}`}>
                  Item
                </th>
                <th className={`px-2 py-2 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? "text-gray-300" : "text-gray-500"}`}>
                  Qty
                ...
                <th className={`text-right text-xs font-medium uppercase tracking-wider ${darkMode ? "text-gray-300" : "text-gray-500"}`}>
                </th>
              </tr>
            </thead>
                ...
                <tr key={index} className={`${darkMode ? "hover:bg-gray-700" : "hover:bg-gray-50"} transition-colors duration-150`}>
                  <td className="px-1 py-1 whitespace-nowrap">
                    <input
                      type="text"
                      value={item.name}
                      onChange={(e) => handleBreakdownItemChange(index, "name", e.target.value)}
                  ...
                  <td className="px-1 py-1 whitespace-nowrap">
                    <input
                      type="number"
                      value={item.quantity || ""}
                      onChange={(e) => handleBreakdownItemChange(index, "quantity", e.target.value)}
                      min="1"
                      className={`w-full p-2 rounded border focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all duration-200 text-sm ${
                        darkMode
                          ? "border-gray-600 bg-gray-700 focus:border-blue-500"
                          : "border-gray-300 focus:border-blue-500"
                      }`}
                  ...
                  <td className="px-1 py-1 whitespace-nowrap">
                    <div className="relative">
                      ...
                      <input
                        type="text"
                        value={item.price || ""}
                        onChange={(e) => handleBreakdownItemChange(index, "price", e.target.value)}
